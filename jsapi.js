...
function NewFloat64(heap, hostValue) {
  const newFloat64 = heap.allocator.float64(hostValue);
  console.log('A 64 bit float is allocated on the heap');
  return newFloat64;
}

function ReadFloat64(wrappedFloat64) {
  console.log('Read 64 bit float from the heap');
  return wrappedFloat64.read();
}

function NewInt32(heap, hostValue) {
  const newInt32 = heap.allocator.int32(hostValue);
  console.log('A 32 bit integer is allocated on the heap');
  return newInt32;
}

function ReadInt32(wrappedInt32) {
  console.log('Read 32 bit integer from the heap');
  return wrappedInt32.read();
}
...
function testJSAPI() {
  ...
  const wrappedLatest = ArrayIndex(
    newWrappedArray,
    ArrayLength(newWrappedArray) - 1,
  );
  ...
  if (
    ArrayIndex(slice10To21, 0).read() !== 6.9 ||
  ...
  if (
    ArrayLength(popped) !== 11 ||
    value.read() !== -10 ||
    ArrayLength(slice10To21) !== 12
  ) {
  ...
  if (ArrayLength(deleted0) !== shouldBeLength - 1) {
    throw new Error(
      'Delete 0 test failed: ' +
        ArrayLength(deleted0) +
        ' vs. ' +
        (shouldBeLength - 1),
    );
  }
